...
# Purpose: validate the AP1302 reset-via-PS-EMIO-GPIO fix hypothesis
# (2026-05-12). 5o ties ap1302_rst_b to fabric peripheral_aresetn so the
# driver's reset_gpio toggle goes to a dead pin; smartcam wires it through
# PS emio_gpio_o[0..1] so power_on() actually resets the chip. This BD
# replicates smartcam's reset wiring while keeping the rest of our 5o
# camera pipeline (mipi_csi2_rx_subsystem + axis chain + v_frmbuf_wr +
# sensor_iic) UNCHANGED so the hypothesis is tested in isolation.

# T1 / DMA / BRAM / smartconnect_hb / smartconnect_idx are all removed
# so synth/route is ~1-1.5 h instead of 4-5 h.

# Usage:
#   vivado -mode batch -source system_top_camtest.tcl -tclargs <build_dir>
...
if {$argc < 1} {
    puts "ERROR: Usage: system_top_camtest.tcl <build_dir>"
    exit 1
}
...
set project_name "camtest_system"
...
# Block design
set bd_name "system_top_camtest"
...
# Camera pipeline IPs - IDENTICAL to system_top.tcl 5o for hypothesis isolation
