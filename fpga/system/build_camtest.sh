...
# Drives system_top_camtest.tcl. Tests the AP1302 reset-via-EMIO-GPIO
...
LOG_DIR="${BUILD_DIR}/camtest-${TIMESTAMP}"
...
VIVADO_TCL="${SCRIPT_DIR}/system_top_camtest.tcl"
if [ ! -f "$VIVADO_TCL" ]; then
    echo "ERROR: system_top_camtest.tcl not found at ${VIVADO_TCL}"
    exit 1
fi
...
if [ "$RUN_FULL" -eq 1 ]; then
    PROJECT_DIR="${VISIONSOC_DIR}/fpga/build/camtest_system"
    PROJECT_FILE="${PROJECT_DIR}/camtest_system.xpr"
