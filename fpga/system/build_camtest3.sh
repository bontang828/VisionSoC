...
# Drives system_top_camtest3.tcl. Tests the AP1302 reset-via-EMIO-GPIO
...
LOG_DIR="${BUILD_DIR}/camtest3-${TIMESTAMP}"
...
VIVADO_TCL="${SCRIPT_DIR}/system_top_camtest3.tcl"
if [ ! -f "$VIVADO_TCL" ]; then
    echo "ERROR: system_top_camtest3.tcl not found at ${VIVADO_TCL}"
    exit 1
fi
...
if [ "$RUN_FULL" -eq 1 ]; then
    PROJECT_DIR="${VISIONSOC_DIR}/fpga/build/camtest3_system"
    PROJECT_FILE="${PROJECT_DIR}/camtest3_system.xpr"
