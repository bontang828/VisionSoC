...
# Purpose: validate the CMN_NUM_PIXELS=2 + CSI_BUF_DEPTH=4096 +
# SAMPLES_PER_CLOCK=2 hypothesis (camtest2, 2026-05-12 evening).
# Camtest1 (reset GPIO fix only) failed: chip emitted frames (HINF
# advancing 30 fps under manual un-stall) but CSI2RX showed 0 packets.
# So reset wiring wasn't the bug. Camtest2 KEEPS the reset GPIO fix
# (no harm; matches smartcam) AND moves to smartcam's canonical
# ppc=2 / buf_depth=4096 / SAMPLES_PER_CLOCK=2 config. Suspicion:
# at ppc=1 the CSI2RX subsystem's data-plane 2:1 deserialize from
# 32-bit DPHY to 16-bit AXIS has an undocumented constraint we're
# violating; at ppc=2 the path is 1:1 (matches DPHY width) and works.

# T1 / DMA / BRAM / smartconnect_hb / smartconnect_idx are all removed
# so synth/route is ~1-1.5 h instead of 4-5 h.

# Usage:
#   vivado -mode batch -source system_top_camtest3.tcl -tclargs <build_dir>
# Launched by build_camtest.sh; does NOT touch the production
# system_top.tcl / system_top_wrapper that 5p is currently building from.

if {$argc < 1} {
    puts "ERROR: Usage: system_top_camtest3.tcl <build_dir>"
    exit 1
}
...
set project_name "camtest3_system"
...
# Block design
set bd_name "system_top_camtest3"
...
# Camera pipeline IPs - camtest3 v5: ppc=2 + buf=4096 (matches smartcam).
# Verified 2026-05-13: ppc=1 silently breaks csiss decoding even with
# FVCO fixed; ppc=2 is the working config.
...
# Match smartcam: just FIFO_DEPTH, let Vivado auto-propagate TUSER/TLAST
# from upstream csiss video_out.
set_property -dict [list \
...
# EXACTLY match smartcam: prepend 16 zero bits to the entire 32-bit input
# (not interleaved per-pixel padding!). frmbuf at SAMPLES_PER_CLOCK=2 expects
# 48-bit beats where bits [47:32]=00, [31:0]=2 UYVY pixels. Tdata-only remap;
...
# v_frmbuf_wr: 5o config (UYVY only, no NV12) - keeps dts compatible with 5o.
# 5p's HAS_Y_UV8_420=1 / MAX_NR_PLANES=2 deferred until camera frames flow.
