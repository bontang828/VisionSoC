...
# Usage:
#   vivado -mode batch -source system_top_camtest2.tcl -tclargs <build_dir>
...
if {$argc < 1} {
    puts "ERROR: Usage: system_top_camtest2.tcl <build_dir>"
    exit 1
}
...
set project_name "camtest2_system"
...
# Block design
set bd_name "system_top_camtest2"
...
# Camera pipeline IPs - camtest2: ppc=2 + buf=4096 to match smartcam
...
# camtest2: at ppc=2 csiss outputs 32-bit AXIS beats (2 pixels × 16 bpp UYVY).
# v_frmbuf_wr at SAMPLES_PER_CLOCK=2 + HAS_UYVY8=1 expects 48-bit beats (2
# pixels × 24-bit UYVY where Vivado pads upper byte per pixel for alignment).
# So input 32 bits, output 48 bits; pad upper byte of each 24-bit pixel slot.
set_property -dict [list \
    CONFIG.M_TDATA_NUM_BYTES {6} \
    CONFIG.M_TDEST_WIDTH     {1} \
    CONFIG.S_TDATA_NUM_BYTES {4} \
    CONFIG.S_TDEST_WIDTH     {10} \
    CONFIG.TDATA_REMAP       {8'b00000000,tdata[31:16],8'b00000000,tdata[15:0]} \
